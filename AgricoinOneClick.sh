...
COINGITHUB=https://github.com/LordMooN/AgricoinMN.git
SENTINELGITHUB=NOSENTINEL
COINSRCDIR=AgricoinMN
...
installDependencies() {
    ...
    sudo apt-get install build-essential libtool autotools-dev autoconf pkg-config libssl-dev libevent-dev   -qq -y > /dev/null 2>&1
...
compileWallet() {
    ...
    chmod +x /root/$COINSRCDIR/src/leveldb/build_detect_platform
...
configureWallet() {
    ...
    echo -e "rpcuser=${rpcuser}\nrpcpassword=${rpcpass}\nrpcallowip=127.0.0.1\nlisten=1\nserver=1\ndaemon=1\nstaking=0\nmaxconnections=64\nlogtimestamps=1\nexternalip=${mnip}:${COINPORT}\nmasternode=1\nmasternodeprivkey=${mnkey}\naddnode=107.191.60.243" > /root/$COINCORE/$COINCONFIG
    echo -e "${NONE}${GREEN}* Completed${NONE}";
}
...
echo -e "${CYAN}                    MMMMMM     MMMMMM                                           ${NONE}"
echo -e "${CYAN}                    MMMMM       MMMMMM                                           ${NONE}"
echo -e "${CYAN}                   MMMMMM       MMMMMM                                          ${NONE}"
...
echo -e "${BOLD}"
read -p "This script will setup your Agricoin Masternode. Do you wish to continue? (y/n)?" response
echo -e "${NONE}"

if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
    read -e -p "Masternode Private Key (e.g. 4edfjLCUzGczZi3JQw8GHp434R9kNY33eFyMGeKRymkB56G4324h) : " key
    if [[ "$key" == "" ]]; then
    ...
    echo
    echo -e "${BOLD}Thank you for your support of Agricoin Masternode.${NONE}"
    echo
else
    echo && echo "Installation cancelled" && echo
fi
